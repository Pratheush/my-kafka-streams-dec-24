package com.mylearning.orderkafkastreamsapp.producer;
...
import com.mylearning.orderkafkastreamsapp.domain.OrderLineItem;
import com.mylearning.orderkafkastreamsapp.domain.OrderType;
...
/**
 * kafka-console-consumer --bootstrap-server localhost:9092 --topic orders-restaurant
 * kafka-console-consumer --bootstrap-server localhost:9092 --topic orders-general
 */
...
//        publishOrdersForGracePeriod(objectMapper, buildOrders());
    ...
    private static List<Order> buildOrders() {
        List<OrderLineItem> orderItemsGeneral = List.of(
    ...
    /**
     * this function is going to change timestamp of records by adding Day 1 from the current timestamp day
     * @param objectMapper
     */
    ...
    /**
     * this function is going to change timestamp of records by subtracting day 1 from the current timestamp day.
     * @param objectMapper
     */
        ...
        orders
                .forEach(order -> {
                    try {
                        ...
                        var recordMetaData = ProducerUtil.publishMessageSync(OrdersTopology.ORDERS, order.orderId()+"", ordersJSON);
